...
function addPointSymbol(props) {
    ...
    function click_symbol(e) {
        ...
        if (isAddSingle) {
            ...
            let color = SuperMap3D.Color.fromCssColorString(state.symbolColor);
            ...
            // let path = '../../../assets/images/addPoint/tree1.s3m';
        ...
        let symbol = scene.pick(e.message.position) || scene.selectedEntity;
        if (symbol && symbol.id && typeof (symbol.id) === 'string' && symbol.id.indexOf("symbol-") != -1) {
            ...
            modelEditor = new SuperMap3D.ModelEditor({
                ...
                scene: scene,
                // offset: new SuperMap3D.Cartesian3(0, 0, 2)
            });
    ...
    function clear() {
        ...
        if (currentUrls.length === 0) {
            scene.eventManager.removeEventListener("CLICK", click_symbol); //移除鼠标点击事件监听
        }
        if (modelEditor) modelEditor.destroy();
        modelEditor = null;
    ...
    function DrawPolylineUpdate(position, line) {
        ...
        scene.clampToHeightMostDetailed(positions)
            .then((Cartesians) => {
                isAddLIneFace = false;
                let color = SuperMap3D.Color.fromCssColorString(state.symbolColor);
    ...
    function statrtAdd() {
        ...
        scene.eventManager.addEventListener("CLICK", click_symbol, true);
