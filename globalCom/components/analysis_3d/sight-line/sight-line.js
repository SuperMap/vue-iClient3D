...
function sightLine(props) {
    ...
    function init() {
        ...
        sightline.visibleColor = SuperMap3D.Color.fromCssColorString(
            state.visibleColor
        );
        sightline.hiddenColor = SuperMap3D.Color.fromCssColorString(state.hiddenColor);

        sightline.lineWidth = Number(state.lineWidth);
        // dragEntity = new DragEntity(scene, 'sightPoint_', _moveEndCallBack, _leftUpCallBack)
    };
    ...
    */
    ...
    watch(() => state.visibleColor, val => {
        if (sightline)
            sightline.visibleColor = SuperMap3D.Color.fromCssColorString(val);
    });
    watch(() => state.hiddenColor, val => {
        if (sightline)
            sightline.hiddenColor = SuperMap3D.Color.fromCssColorString(val);
    });
