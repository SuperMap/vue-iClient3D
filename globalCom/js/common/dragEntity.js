class DragEntity {
    ...
    addEventListener() {
        ...
        that.viewer.screenSpaceEventHandler
            .setInputAction(leftDownAction, SuperMap3D.ScreenSpaceEventType.LEFT_DOWN);
        that.viewer.screenSpaceEventHandler
            .setInputAction(leftUpAction, SuperMap3D.ScreenSpaceEventType.LEFT_UP);
        that.viewer.screenSpaceEventHandler
            .setInputAction(mouseMoveAction, SuperMap3D.ScreenSpaceEventType.MOUSE_MOVE);

        function leftDownAction(e) {
            let picked = that.viewer.scene.pick(e.position);
            that.leftDownFlag = true;
            if (picked) {
                let Entity = SuperMap3D.defaultValue(picked.id, picked.primitive.id);
