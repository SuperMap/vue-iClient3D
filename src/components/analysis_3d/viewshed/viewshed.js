...
function viewshed(props) {
    ...
    */
    ...
    function getVisibleResult() {
        ...
        let color = SuperMap3D.Color.fromCssColorString(state.visibleBodyColor);
            ...
            .then(function (response) {
                ...
                window.axios
                    ...
                    .then(function (response) {
                        ...
                        viewshed3D.hiddenAreaColor = SuperMap3D.Color.fromCssColorString("rgba(0,0,0,0)");
                        // // 传到store可以做gpu查询
    ...
    function getInVisibleResult() {
        ...
        let queryData = JSON.stringify(geometryViewShedBodyvisibleParameter);
        let color = SuperMap3D.Color.fromCssColorString(state.invisibleBodyColor);
        //先发送POST请求
        window.axios
            .post(state.viewshedSpatialUrl, queryData)
            .then(function (response) {
    ...
    */
    ...
    watch(() => state.viewshedAnimation, val => {
        if (val) {
            clearViewshed();
            tooltip.showAt('<p>点击分析按钮激活事件</p><p>点击鼠标左键确认模型移动路线</p><p>单击右键结束分析</p>', '450px');
            setTimeout(()=>{
                tooltip.setVisible(false);
            },3000)
        } else {
