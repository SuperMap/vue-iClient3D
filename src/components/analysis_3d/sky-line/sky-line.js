...
function skyLine(props) {
    ...
    scene = viewer.scene;
    s3mInstance = new SuperMap3D.S3MInstanceCollection(viewer.scene._context);
    viewer.scene.primitives.add(s3mInstance);

    skyline = new SuperMap3D.Skyline(viewer.scene);
    ...
    */
    ...
    function skyLineUpdate(observerObj) {
        ...
        if(skyline.displayStyle === 1) updatePosition();
    }
    ...
    watch(() => state.ignoreGlobe, newValue => {
        ...
        skyLineAnalysis();//上层规避掉天际线遗留问题 - 即重新生成天际线
    });
