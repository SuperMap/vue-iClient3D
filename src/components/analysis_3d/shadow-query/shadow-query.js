...
function shadowquery(props) {
    ...
    let state = reactive({
        ...
        layerShadowType: SuperMap3D.ShadowType.ALL, // 图层上阴影类型：所有的模型都产生阴影。
        terrainShadows: SuperMap3D.ShadowMode.RECEIVE_ONLY, //确定地形是否投射或接受来自太阳的阴影:(ShadowMode:该对象仅接收阴影)。
    ...
    function init() {   //初始化函数
        ...
        shadowQuery = new SuperMap3D.ShadowQueryPoints(scene);    //创建阴影查询对象
        s3mInstanceCollection = new SuperMap3D.S3MInstanceCollection(scene._context);
    ...
    */
    ...
    function timeChanged(val) {
        if (state.shadowShow) {
            shadowQuery.startTime = SuperMap3D.JulianDate.fromDate(setCurrentTime(val[0]));
            shadowQuery.endTime = SuperMap3D.JulianDate.fromDate(setCurrentTime(val[1]));
        }
    ...
    function sunLightForDate(flag) {
        ...
        IntervalForDate = setInterval(() => {
            ...
            d.setMonth(mon);
            // viewer.clock.currentTime = SuperMap3D.JulianDate.fromDate(d);
    ...
    function shadowQueryUpdate(p) {
        ...
        for (let i = 0, len = pos.length; i < len; i++) {
            //转化为经纬度，并加入至临时数组
            let cartographic = SuperMap3D.Cartographic.fromCartesian(p[i]);
    ...
    function bodyShow() {
        ...
        for (let i = 0, j = shadowQuery_points.length; i < j; i++) {
            objs.push({
                ...
                scale: new SuperMap3D.Cartesian3(width, width, width)
            })
        };
        if (objs.length > 0) {
