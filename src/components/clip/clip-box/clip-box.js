    ...
    */
    ...
    function setAllLayersClipOptions(boxOptions) {
        for (let layer of layers) {
            hasClipLine
                ? (layer.clipLineColor = new SuperMap3D.Color(1, 1, 1, 1))
    ...
    function setAllLayersClipColor() {
        for (let i = 0, j = layers.length; i < j; i++) {
            ...
            layers[i].clipLineColor = new SuperMap3D.Color(1, 1, 1, 0.5);
        }
    };
    // 更新
    function ClipBoxUpdate() {
        ...
        let heading = SuperMap3D.Math.toRadians(parseFloat(state.clipRotate));
        let boxOptions = {
            dimensions: newDim,
            position: position,
            clipMode: clipMode,
    ...
    watch(() => state.clipLength, val => {
        ...
        boxEntity.box.dimensions = new SuperMap3D.Cartesian3(
            newVal,
            dim.y,
            dim.z
        );
    ...
    watch(() => state.clipRotate, val => {
        ...
        let position = boxEntity.position.getValue(0);
        let newVal = Number(val);
        let rotate = SuperMap3D.Math.toRadians(newVal);
        let hpr = new SuperMap3D.HeadingPitchRoll(rotate, 0, 0);
        let orientation = SuperMap3D.Transforms.headingPitchRollQuaternion(
            position,
            hpr
