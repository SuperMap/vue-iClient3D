   ...
   */
    ...
    watch(() => state.selectedAddedStopIndex, val => {
        ...
        viewer.camera.setView({
            destination: SuperMap3D.Cartesian3.fromDegrees(stop.point[0], stop.point[1], stop.point[2]),
            orientation: {
    ...
    watch(() => state.routeType, val => {
        if(val === 'customRoute'){
            window.tooltip.showAt(' <p>调整当前相机位置和视角</p><p>以当前相机位置和视角设置站点</p><p>点击添加保存此站点</p>', '80%');
            setTimeout(()=>{
