...
function projectionImage(props) {
    ...
    watch(() => state.hintLineColor, val => {
        if (!currentProject) return;
        currentProject.hintLineColor = SuperMap3D.Color.fromCssColorString(val);
    })
    ...
    watch(() => state.fromInfo, val => {
        if (!val || val == "") return;
        SuperMap3D.ProjectionImage.fromInfo(scene, state.fromInfo.infoUrl, state.fromInfo.baseUrl)
    })
