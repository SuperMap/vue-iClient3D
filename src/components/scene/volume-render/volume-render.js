...
function volume(props) {
    ...
    // 初始化数据
    let gridLayer;
    let scaleScratch = new SuperMap3D.Cartesian3(1, 1, 1); //z拉伸
    let scratchCartesian3 = new SuperMap3D.Cartesian3(0.5, 0.5, 0.5); //偏移
    let scratchClipPlane = SuperMap3D.Plane.fromPointNormal(new SuperMap3D.Cartesian3(0, 0, 0), new SuperMap3D.Cartesian3(1, 0, 0)); //裁剪面
    let colorTable;
    ...
    function init() {
        ...
        SuperMap3D.when(promise, function (layer) {
            ...
            state.yDim = state.dimensions[1].name;
            state.zDim = state.dimensions[2].name;
            state.timeDim = state.dimensions[3].name;

            //默认体渲染位置
            gridLayer.layerBounds = new SuperMap3D.Rectangle(99.95422, 32.25671, 107.1912, 37.1386);
            gridLayer.zBounds = new SuperMap3D.Cartesian2(500, 50000);
        })
    }
    ...
    function addClipPlane() {
        if (!SuperMap3D.defined(gridLayer) || gridLayer.getNumberOfClipPlanes() > 0) {
            return;
        }
    ...
    function addPoint() {
        ...
        viewer.entities.add({
            ...
            position: SuperMap3D.Cartesian3.fromDegrees(lat, lng, 50000),
            point: {
    ...
    watch(() => state.clipX, val => {
        ...
        let newRot = SuperMap3D.Math.toRadians(state.clipRotate);
        let qua = SuperMap3D.Quaternion.fromAxisAngle(SuperMap3D.Cartesian3.UNIT_Z, newRot);
        let pos = new SuperMap3D.Cartesian3(newPosX, newPosY, 0.0);
        let transRotScale = new SuperMap3D.TranslationRotationScale(pos, qua);
        let transformMatrix = SuperMap3D.Matrix4.fromTranslationRotationScale(transRotScale);
        SuperMap3D.Plane.fromPointNormal(new SuperMap3D.Cartesian3(0, 0, 0), new SuperMap3D.Cartesian3(1, 0, 0), scratchClipPlane);
        SuperMap3D.Plane.transform(scratchClipPlane, transformMatrix, scratchClipPlane);
    });
    watch(() => state.clipY, val => {
        ...
        var pos = new SuperMap3D.Cartesian3(newPosX, newPosY, 0.0);
        SuperMap3D.Plane.fromPointNormal(pos, new SuperMap3D.Cartesian3(0, 1, 0), scratchClipPlane);
    });
    watch(() => state.clipRotate, val => {
        if (!gridLayer) return;
        var newPosX = parseFloat(state.clipX);
        var newPosY = parseFloat(state.clipY);
        var newRot = SuperMap3D.Math.toRadians(val);
        var qua = SuperMap3D.Quaternion.fromAxisAngle(SuperMap3D.Cartesian3.UNIT_Z, newRot);
        var pos = new SuperMap3D.Cartesian3(newPosX, newPosY, 0.0);
        var rotMatrix3 = SuperMap3D.Matrix3.fromQuaternion(qua);
        var rotMatrix4 = SuperMap3D.Matrix4.fromRotationTranslation(rotMatrix3);
        var normal = new SuperMap3D.Cartesian3(1, 0, 0);
        normal = SuperMap3D.Matrix4.multiplyByPoint(rotMatrix4, normal, new SuperMap3D.Cartesian3());
        SuperMap3D.Plane.fromPointNormal(pos, normal, scratchClipPlane);
    });
    // 剖切模式
