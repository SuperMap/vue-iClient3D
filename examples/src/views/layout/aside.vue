        ...
        layerManagement.addScene(
          ...
          layers => {
            ...
            if (obj.style && obj.style.fillStyle) {
              layers[0].style3D.lineColor = SuperMap3D.Color.fromCssColorString(
                "rgb(67,67,67)"
              );
              layers[0].style3D.fillStyle =
                SuperMap3D.FillStyle[obj.style.fillStyle];
            }
          }
        );
    ...
    addBaseLayer(obj) {
      ...
      switch (type) {
        ...
        case "OSM":
          ...
          break;
        case "MAPBOX":
          imageryProvider = new SuperMap3D.MapboxImageryProvider({
            mapId: "mapbox.dark"
          });
          break;
        case "SUPERMAPDARK":
      ...
      if (type == "GRIDIMAGERY") {
        imageryLayerCollection.addImageryProvider(
          new SuperMap3D.TileCoordinatesImageryProvider(),
          1
        );
        imageryLayerCollection.addImageryProvider(
          new SuperMap3D.GridImageryProvider(),
          2
        );
      }
    },
    ...
    addOnlineTerrain(obj) {
      ...
      switch (type) {
        ...
        case "tianDiTuTerrain":
          var t_Provider = new SuperMap3D.TiandituTerrainProvider({
